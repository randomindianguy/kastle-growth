...
const C = {
  ...
  glassHover: "linear-gradient(135deg, rgba(255,255,255,0.92) 0%, rgba(240,237,252,0.88) 100%)",
...
function LandscapeTab() {
  ...
  const funnelStages = [
    ...
    { stage: "Consideration", question: "What alternatives does Kastle compete against?", width: "82%",
      ...
      insight: "The comparison set is a choice, not a given. 'Compliance-first AI workforce' competes against BPOs (where Kastle wins easily) rather than other AI platforms (where differentiation is harder)." },
    { stage: "Conversion", question: "What tips the deal in Kastle's favor?", width: "60%",
      kastle: "70% lower cost per call. 40% lower handle time. 90%+ CSAT. Named logos (Newrez, Angel Oak, Federal Savings Bank). 7-day go-live. No rip-and-replace.",
      ...
      insight: "Enterprise buyers need proof, not promises. Named logos + 7-day go-live directly attacks the two biggest objections: 'does this actually work?' and 'how long will implementation take?'" },
  ...
  return (
    <div>
      ...
      {activeSection === "position" && (
        <div>
          <GlassCard glow>
            <SectionLabel>The positioning choice that matters</SectionLabel>
            <div style={{ fontSize: 14.5, color: C.text, lineHeight: 1.65, fontFamily: FONT.body, marginBottom: 16 }}>
              Kastle positions as <span style={{ fontWeight: 600, color: C.accent }}>"compliance-first AI workforce for servicing"</span> — not "AI voice agent for mortgage." That one word difference changes the entire competitive landscape.
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div style={{ padding: "14px 16px", background: "rgba(0,0,0,0.02)", borderRadius: 10, border: `1px solid ${C.borderOuter}` }}>
                <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: 1.5, color: C.textDim, fontFamily: FONT.mono, fontWeight: 600, marginBottom: 6 }}>If positioned as "AI voice agent"</div>
                <div style={{ fontSize: 12.5, color: C.textMuted, lineHeight: 1.55, fontFamily: FONT.body }}>Compared to: Marr Labs, Dialpad, Observe.AI. Fight is on technology — model quality, latency, voice realism. Hard to differentiate.</div>
              </div>
              <div style={{ padding: "14px 16px", background: C.accentMuted, borderRadius: 10, border: `1px solid ${C.borderGlow}` }}>
                <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: 1.5, color: C.accent, fontFamily: FONT.mono, fontWeight: 600, marginBottom: 6 }}>If positioned as "compliance-first workforce"</div>
                <div style={{ fontSize: 12.5, color: C.text, lineHeight: 1.55, fontFamily: FONT.body }}>Compared to: manual BPOs, in-house teams, legacy IVR. Fight is on cost, speed, and audit-readiness. Kastle wins easily.</div>
              </div>
            </div>
          </GlassCard>
...
function PlanTab() {
  ...
  const weeks = [
    { week: "Week 1", title: "Map the machine",
      ...
      why: "Before building anything, I need to understand the full lifecycle from first demo to first AI call. An 8-person team means this knowledge lives in people's heads — documenting it creates leverage.",
      actions: [
